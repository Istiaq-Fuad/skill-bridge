...
public class ResumeParsingController {
     ...
     * 
     * @param file   The uploaded resume file
     * @param userId The user ID to save the data to (optional, defaults to current
     *               user)
        ...
        try {
            ...
            if (targetUserId == null) {
                ...
                if (currentUserOpt.isPresent()) {
                    targetUserId = currentUserOpt.get().getId();
                } else {
                    return ResponseEntity.badRequest()
                            .body(new ErrorResponse("User Error", "Unable to find current user"));
                }
            }
            ...
            // Save to user profile - this would need to be implemented in UserService
            // For now, just return the parsed data

            return ResponseEntity.ok(new SuccessResponse("Resume parsed successfully", parsedData));
        } catch (AiServiceException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse("Parsing Error", e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Internal Server Error",
                            "An unexpected error occurred while parsing and saving the resume"));
        }
    }
    ...
    public ResponseEntity<?> extractSkills(@RequestParam("file") MultipartFile file) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Internal Server Error",
                            "An unexpected error occurred while extracting skills"));
        }
    }
    ...
    public ResponseEntity<?> extractExperiences(@RequestParam("file") MultipartFile file) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Internal Server Error",
                            "An unexpected error occurred while extracting experiences"));
        }
    }
    ...
    public ResponseEntity<?> extractEducation(@RequestParam("file") MultipartFile file) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Internal Server Error",
                            "An unexpected error occurred while extracting education"));
        }
    }
